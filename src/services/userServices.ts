...
export const loginApi = async (data: LoginData) => {
    const response: AxiosResponse<User> = await customAxios.post('/api/auth/signin', { ...data, user_type: "mentor" })
    return response
}

export const signApi = async (data: MentorFields & UserFields) => {
    const response: AxiosResponse<{ message: string }> = await customAxios.post('/api/auth/signup', data, {
        headers: {
